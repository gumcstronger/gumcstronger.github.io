...
基本参考Windows，Mac部分特有的才在这里

### 系统软件

  * 键盘修改
    * [Karabiner-Elements](https://github.com/pqrs-org/Karabiner-Elements) 将常见的command快捷键例如复制粘贴保存修改为ctrl
  * iPad做第二屏幕
    * ~~[Yam Display](https://www.yamdisplay.com/) 将ipad作为mac的第二屏幕（免费版7分钟重连）~~
    * ~~[Deskpad](https://github.com/Stengo/DeskPad)+ [Deskreen](https://github.com/pavlobu/deskreen) Mac虚拟屏幕和第二屏幕，分辨率和延迟严重~~
    * Duet Display 免费版通过USB连接，每30分钟重连
...
### 装机软件
  ...
  * [sideloadly](https://sideloadly.io/) iOS不越狱安装ipa
...
#### Mac为Blener配置命令
  ...
  * blender安装后查看是否在PATH中

```bash
which blender
# 显示 blender not found
```
...
# 如果输出是 /bin/bash（较旧的 macOS 系统），你需要编辑 ~/.bash_profile 文件
