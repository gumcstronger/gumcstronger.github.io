...
右上角的视图层，通过筛选选项，去掉空物体，然后就可以通过a键全选了
...
* sketchfab导出模型后发生问题：
  ...
  * 在Blender的Texture Paint颜色是正确的，导入Unity后颜色错误，因为Texture被Compression，将Compression设置为None则每问题

## 导入Unity

* .Blend文件或fbx文件导入Unity，Bounds与实际不符，如何修复:
  * 应用所有变换 (Apply All Transforms): 选中该Mesh物体，Ctrl + A -> All Transforms, 确保其在物体模式下的Scale是(1,1,1)，Rotation是(0,0,0)，Location是你期望的（或者应用它）。这是最最关键的一步。
  * 设置物体原点 (Set Object Origin) : Object -> Set Origin -> Origin to Center Of Mass(Surface)。确保原点位于中心。
